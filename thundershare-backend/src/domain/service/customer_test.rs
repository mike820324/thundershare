...
use crate::domain::{entity::customer::Customer, error::customer::CustomerError, repository::customer::MockCustomerRepositoryTrait};
...
async fn test_customer_svc_signup() {
        ...
        CustomerSignUpTestContext::new(
            ...
            || {
                let mock_customer_repo = {
                    ...
                    .times(1)
                    .returning(move |_username| {
                        Ok(vec![])
                    });

                    mock_repo.expect_create_customer()
                    .times(1)
                    .returning(|username, password| {
                        Ok(Customer::new(username, password))
                    });
    ...
    for t in test_context {
        let svc = (t.setup_fn)();
        let result = svc.customer_signup(&t.input_customer.get_username(), &t.input_customer.get_password()).await.map_err(|err| err.downcast().unwrap());
...
async fn test_customer_svc_signin() {
        ...
        CustomerSignInTestContext::new(
            ...
            || {
                let mock_customer_repo = {
                    ...
                    mock_repo.expect_get_customer_by_credential()
                    .times(1)
        ...
        CustomerSignInTestContext::new(
            ...
            || {
                let mock_customer_repo = {
                    ...
                    mock_repo.expect_get_customer_by_credential()
                    .times(1)
                    .returning(move |_username, password| {
                        Ok(vec![])
                    });
                    mock_repo
                };
    ...
    for t in test_context {
        let svc = (t.setup_fn)();
        let result = svc.customer_signin(&t.input_customer.get_username(), &t.input_customer.get_password()).await.map_err(|err| err.downcast().unwrap());
